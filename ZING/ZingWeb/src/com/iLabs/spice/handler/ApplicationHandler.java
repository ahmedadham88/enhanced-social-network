...
public class ApplicationHandler extends PageCodeBase {
	 ...
	 */
	private void updateActivity(int applicationId, int userId) throws FileNotFoundException, IOException {
	...
	public void addApplication() {
		...
		if (userApplication == null) {
			FacesMessage message = new FacesMessage("Check if valid Gadget URL!!");
	...
	public String postInApplication() {
		Application userApplication = (Application) getRequestScope().get(
				"applicationBean");
		ProfileBean currentProfile = (ProfileBean) getSessionScope().get(
				"currentProfile");
		String url = userApplication.getUrl();
		String postedText=userApplication.getUrl();
		//userApplication = parseURL(url);
...
//		}
		if (postedText.isEmpty()) {
			FacesMessage message = new FacesMessage("Check if not null!!");
			System.out.println("postedText is null");
			getFacesContext().addMessage("addApplication", message);
			return "";
		}	
...
//						"applicationId").toString());
				...
				System.out.println("Posting 101 ... ");
				
				boolean result = false;//ContentFilter.Filter(postedText,"Food and Agriculture");
				System.out.println("Result of Filter: "+result);
		    	if(result==true)
		    		updatePostActivity("This is filtered", userId);
		    	else
		    		updatePostActivity(postedText, userId);
...
//		}
	...
	private void updatePostActivity(String postedText, int userId) throws FileNotFoundException, IOException {
		IActivity activity = null;
		Activity activityObj = null;
		ProfileBean ownerProfile = null;
		UserAuth authPerson = null;
		...
		try {
			...
			activityObj.setApplicationId(0);
			
			activityObj.setBody(postedText);
			
			//String title = (String) getRequestParam().get("applicationTitle");
			
			activityObj.setTitle(" has posted ");
