...
//import com.iLabs.filter.ContentFilter;
...
/*
     ...
     * 
     */
    public List<Activity> getAllFriendActvityList() throws FileNotFoundException, IOException {
        if (allFriendActvityList == null) {
            allFriendActvityList = new ArrayList<Activity>();
        }
        else{
        
    		ProfileTaskHandler pth = new ProfileTaskHandler();
    		String contentFilterUser = pth.getContentFilter();
    		if(contentFilterUser.equalsIgnoreCase("Food and Agriculture")){
    			System.out.println("BINGO "+contentFilterUser+" size "+ allFriendActvityList.size());
    			TCPClient tcp = new TCPClient();
    			tcp.Connect();
    			for(int i =0; i<allFriendActvityList.size();i++){
    				
	    			String toFilter = allFriendActvityList.get(i).getBody();
	    			String answer = tcp.SocketWrite(toFilter);
	    //			if(toFilter.startsWith("Food")){
	    			if(answer.equalsIgnoreCase("yes")){
	    				allFriendActvityList.remove(i);
	    			}
    				answer = allFriendActvityList.get(i).getBody();
    				if(answer.endsWith("food")){
    					allFriendActvityList.get(i).setBody("Filtered Sentence");
    				}
    	//		}
    			tcp.CloseConnect();
    		}
