/*
 * $Id: AVP.java 361 2007-06-28 21:07:47Z vingarzan $
 *
...
public class AVP {
	...
	protected void setSingleAVP(AVP _avp) {
		AVP current = this.findChildAVP(_avp.code);
		if (current != null)
			this.deleteChildAVP(current);
		this.addChildAVP(_avp);
	}
	 ...
	 * 
	 * @param <T>
	 *            the requested AVP type
	 * @param t
	 *            a dummy Object of the Type T to enable compares
	 * @return a Iterator with all AVPs of the Type T
	 * @author mhappenhofer
	 */
	...
	public <T> Iterator<T> getIterator(T t) {
		...
		if (childs != null) {
			for (Iterator<AVP> it = childs.iterator(); it.hasNext();) {
				AVP avp = it.next();
				if (u.isInstance(avp)) {
					T found = (T) avp;
					temp.add(found);
